...
void LanPairingHandler::packageReceived(const NetworkPackage& np)
{
    ...
    if (wantsPair) {
        ...
        if (isPairRequested())  { //We started pairing
            ...
            qCDebug(coreLogger) << "Pair answer";
            setInternalPairStatus(Paired);
            
        } else {
            qCDebug(coreLogger) << "Pair request";
        ...
        qCDebug(coreLogger) << "Unpair request";

         if (isPairRequested()) {
            Q_EMIT pairingError(tr("Canceled by other peer"));
        }
...
{
    if (m_status == Paired) {
        Q_EMIT pairingError(tr("%1: Already paired").arg(deviceLink()->name()));
        return false;
    }
    if (m_status == RequestedByPeer) {
        qCDebug(coreLogger) << deviceLink()->name() << ": Pairing already started by the other end, accepting their request.";
...
void LanPairingHandler::pairingTimeout()
{
    ...
    Q_EMIT pairingError(tr("Timed out"));
}
